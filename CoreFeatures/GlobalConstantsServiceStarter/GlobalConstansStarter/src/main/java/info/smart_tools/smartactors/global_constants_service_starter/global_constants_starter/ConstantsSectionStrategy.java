...
public class ConstantsSectionStrategy implements ISectionStrategy {
    ...
    public ConstantsSectionStrategy()
        ...
        sectionFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "const");
        constantNameFN = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "name");
        constantValueFN = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "value");
    }

    @Override
    public void onLoadConfig(final IObject config) throws ConfigurationProcessingException {
        try {
            IObject constants = IOC.resolve(Keys.resolveByName("global constants"));
            List<IObject> section = (List<IObject>) config.getValue(sectionFieldName);

            for (IObject obj : section) {
                IFieldName cFName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), obj.getValue(constantNameFN));
