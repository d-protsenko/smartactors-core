...
public class HttpRequestHandler extends EndpointHandler<ChannelHandlerContext, FullHttpRequest> {
    ...
    public HttpRequestHandler(
        ...
        httpResponseIsSentFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "sendResponseOnChainEnd");
        httpResponseStatusCodeFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "httpResponseStatusCode");
        accessForbiddenFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "accessToChainForbiddenError");
        requestFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "request");
        channelFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "channel");
        ...
        endpointName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "endpointName");
        responseStrategyName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "responseStrategy");
    ...
    protected IObject getEnvironment(final ChannelHandlerContext ctx, final FullHttpRequest request)
        ...
        try {
            IObject message = IOC.resolve(Keys.resolveByName("EmptyIObject"));
            if (!request.method().toString().equals("GET")) {
                IDeserializeStrategy deserializeStrategy = IOC.resolve(
                        ...
                        IOC.resolve(Keys.resolveByName("http_request_key_for_deserialize"), request),
                        name
                );
                ...
                try {
                    ...
                    IObject exception = IOC.resolve(Keys.resolveByName("HttpPostParametersToIObjectException"));
            ...
            try {
                ...
                IObject exceptionalResponse = IOC.resolve(Keys.resolveByName("HttpRequestParametersToIObjectException"));
            ...
            IChannelHandler channelHandler = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.http_endpoint.channel_handler_netty.ChannelHandlerNetty"), ctx);
            ...
            context.setValue(responseStrategyName, IOC.resolve(Keys.resolveByName("endpoint response strategy")));
            ...
            try {
                IObject exception = IOC.resolve(Keys.resolveByName("HttpInternalException"), e);
                ctx.writeAndFlush(formExceptionalResponse(exception));
    ...
    private FullHttpResponse formExceptionalResponse(final IObject iObjectResponse)
        ...
        IFieldName exceptionalStatusCode = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "statusCode");
    ...
    public void handle(final ChannelHandlerContext ctx, final FullHttpRequest request) throws ExecutionException {
        if (isShuttingDown) {
            try {
                sendExceptionalResponse(ctx, request, IOC.resolve(Keys.resolveByName("HttpShuttingDownException")));
            } catch (Exception e1) {
                e1.printStackTrace();
            }
        } else {
