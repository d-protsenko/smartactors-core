...
public class CheckpointConfigCanonizationStrategyPlugin extends BootstrapPlugin {
    ...
    public void registerCanonizationStrategies()
        ...
        IOC.register(Keys.resolveByName("checkpoint step configuration from checkpoint section"), new ApplyFunctionToArgumentsStrategy(args -> {
            try {
                ...
                IFieldName handlerFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "handler");
                IFieldName wrapperFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "wrapper");
        ...
        IAdditionDependencyStrategy ads = IOC.resolve(Keys.resolveByName("expandable_strategy#resolve key for configuration object"));
                ...
                try {
                    ...
                    if (obj instanceof List) {
                        for (IObject innerObject : (List<IObject>) obj) {
                            if (null == innerObject.getValue(IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "externalAccess"))) {
                                innerObject.setValue(IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "externalAccess"), false);
                            }
                            if (!innerObject.getValue(
                                    IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "id"))
                                    .equals("tryToTakeResourceMap")) {
                                List exceptionalList = (List) innerObject.getValue(
                                        IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "exceptional"));

                                IObject outOfResourcesExceptionObj = IOC.resolve(Keys.resolveByName("configuration object"));
                                outOfResourcesExceptionObj.setValue(
                                        IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "class"),
                                        "info.smart_tools.smartactors.base.interfaces.iresource_source.exceptions.OutOfResourceException");
                                outOfResourcesExceptionObj.setValue(
                                        IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "chain"),
                                        "tryToTakeResourceMap");
                                outOfResourcesExceptionObj.setValue(
                                        IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "after"),
                                        "break");
                    ...
                    if (obj instanceof List) {
                        for (IObject innerObject : (List<IObject>) obj) {
                            IFieldName checkpointFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "checkpoint");
                            IFieldName stepsFieldName = IOC.resolve(Keys.resolveByName("info.smart_tools.smartactors.iobject.ifield_name.IFieldName"), "steps");

                            Object checkpointConf = innerObject.getValue(checkpointFieldName);
                            if (null != checkpointConf) {
                                Object checkpointStep = IOC.resolve(
                                        Keys.resolveByName("checkpoint step configuration from checkpoint section"), checkpointConf);
