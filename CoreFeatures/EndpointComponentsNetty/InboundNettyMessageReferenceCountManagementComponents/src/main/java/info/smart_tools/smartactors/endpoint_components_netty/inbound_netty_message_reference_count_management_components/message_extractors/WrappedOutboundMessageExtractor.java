...
/**
 * Function that extracts a outbound wrapped Netty reference counted message from message context.
 *
 * @param <T> Netty message type
 * @param <TCtx> context type
 */
public class WrappedOutboundMessageExtractor
        <T extends IMessageByteArray<? extends ReferenceCounted>, TCtx extends IDefaultMessageContext<?, ? extends IMessageByteArray<T>, ?>>
        implements IFunction<TCtx, T> {
    @Override
    public T execute(final TCtx ctx)
            throws FunctionExecutionException, InvalidArgumentException {
        return ctx.getDstMessage().getMessage();
    }
}
