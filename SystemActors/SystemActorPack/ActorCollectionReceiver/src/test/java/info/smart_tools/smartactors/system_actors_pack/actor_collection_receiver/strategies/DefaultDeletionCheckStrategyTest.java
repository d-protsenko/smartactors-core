...
public class DefaultDeletionCheckStrategyTest extends PluginsLoadingTestBase {
    ...
    public void Should_checkDeletionFlag()
        ...
        IObject ctx = IOC.resolve(Keys.resolveByName(IObject.class.getCanonicalName()));
        IObject env = IOC.resolve(Keys.resolveByName(IObject.class.getCanonicalName()),
                "{'context':{'deleteChild':true}}".replace('\'','"'));

        assertTrue(strategy.checkDelete(ctx, env));

        env = IOC.resolve(Keys.resolveByName(IObject.class.getCanonicalName()),
